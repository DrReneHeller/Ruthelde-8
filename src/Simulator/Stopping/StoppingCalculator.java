...
final public class StoppingCalculator {
    ...
    public StoppingCalculator(StoppingParaFile stoppingParaFile, CorrectionFactorEntry[] _correctionFactors) {
        ...
        //Inject variation of correction (scaling) factor for optimization purpose
        if (_correctionFactors != null){

            int numEntries = _correctionFactors.length;

            if (numEntries > 0){
                for (CorrectionFactorEntry cfo : _correctionFactors){
                    correctionFactors[cfo.Z-1] =  cfo.correctionFactor;
                }
            }
        }
    }
    ...
    private double calcElectronicStoppingZB(int Z1, double M1, int Z2, double E0, StoppingCalculationMode calcMode) {
        ...
        if (calcMode.equals(StoppingCalculationMode.PARA_FILE)){
            ...
            double _a1 = paraFile.data[Z2].params[3];
            double _a2 = paraFile.data[Z2].params[4];
            double _a3 = paraFile.data[Z2].params[5];
            double _f1 = paraFile.data[Z2].params[6];
            double _q  = paraFile.data[Z2].params[7];
            double _r  = paraFile.data[Z2].params[8];
            double _b0 = paraFile.data[Z2].params[9];
            double _b1 = paraFile.data[Z2].params[10];
